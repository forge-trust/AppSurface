...
/// <param name="RawResultsDirectory">The unique AppSurface-owned results directory, or <see langword="null"/> for caller-owned MSBuild results.</param>
...
internal static class CoverageRunDriverStrategy
{
    ...
    /// <summary>
    /// Reserves MSBuild result-directory ownership before discovery in fail mode when the user did
    /// not claim the blame policy. This guard also applies if the later launch budget is too short.
    /// </summary>
    internal static void ValidateHangArguments(CoverageRunRequest request)
    {
        if (!CoverageRunHangPolicy.OwnsMsbuildResults(request))
        {
            return;
        }

        for (var index = 0; index < request.TestArguments.Count; index++)
        {
            var argument = request.TestArguments[index];
            if (argument == "--")
            {
                break;
            }

            if (string.Equals(argument, "--results-directory", StringComparison.OrdinalIgnoreCase))
            {
                throw CoverageRunDiagnostics.Create(
                    "ASCOV101",
                    index + 1 == request.TestArguments.Count || request.TestArguments[index + 1] == "--"
                        ? "--test-argument contains an incomplete owned option."
                        : "--test-argument cannot override AppSurface-owned hang results.",
                    "MSBuild automatic VSTest hang diagnostics own --results-directory.",
                    "Remove the results-directory override or choose --watchdog warn/off or manual blame.",
                    "Cli/ForgeTrust.AppSurface.Cli/README.md#coverage-run-watchdog");
            }

            if (argument.StartsWith("--results-directory=", StringComparison.OrdinalIgnoreCase)
    ...
    /// <param name="hangPlan">The fixed VSTest timeout plan computed immediately before the project operation.</param>
    ...
    public static CoverageRunDriverInvocation CreateInvocation(
        ...
        string projectOutputDirectory,
        CoverageRunHangPlan? hangPlan = null)
    {
        if (request.CoverageDriver == CoverageRunDriver.Msbuild)
        {
            ...
            if (!CoverageRunHangPolicy.OwnsMsbuildResults(request)
                || (hangPlan ?? CoverageRunHangPolicy.Plan(request)).Source != CoverageRunHangSource.Automatic)
            {
                return new CoverageRunDriverInvocation(request.CoverageDriver, projectOutputDirectory, null, args);
            }

            var msbuildDirectory = CreateUniqueResultsDirectory(projectOutputDirectory, "hang-results");
            args.Add("--results-directory");
            args.Add(Path.GetFullPath(msbuildDirectory));
            return new CoverageRunDriverInvocation(request.CoverageDriver, projectOutputDirectory, msbuildDirectory, args);
        }

        var rawResultsDirectory = CreateUniqueResultsDirectory(projectOutputDirectory, "collector-results");
    ...
    private static string CreateUniqueResultsDirectory(string projectOutputDirectory, string kind)
    {
        var runId = RandomNumberGenerator.GetHexString(32).ToLowerInvariant();
        var directory = Path.Join(projectOutputDirectory, kind, runId);
        if (Directory.Exists(directory) || File.Exists(directory))
        {
            throw ArtifactFailure(projectOutputDirectory, directory, "invocation directory collision");
        }
