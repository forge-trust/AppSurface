...
For deployment extras such as `CNAME`, keep the AppSurface Docs exact release tree clean until `.appsurface-docs-release-manifest.json` is written and pinned. Copy deployment-owned files into the surrounding publish root after export or upload preparation; do not place opaque extras inside immutable exact release archives unless a future archive contract explicitly supports them. The RazorWire CLI [Static website deployment extras](../../Web/ForgeTrust.RazorWire.Cli/README.md#static-website-deployment-extras) section has the shared copy-and-verify recipe.
