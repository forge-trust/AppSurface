...
public sealed class ProgramEntryPointTests
{
    ...
    public async Task EntryPoint_Should_Generate_DurableSchemaScript_Offline_With_ParsedFromVersion()
    {
        ...
        Assert.Equal(
            """
            -- Generated by ForgeTrust.AppSurface.Durable.PostgreSql.
            -- Apply with the migration owner. Runtime roles must not execute this script.
            -- Stop on the first error; psql callers must pass -v ON_ERROR_STOP=1.
            SELECT pg_advisory_lock(4707181168775217740);
            SELECT pg_advisory_unlock(4707181168775217740);
