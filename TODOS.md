...
- **What:** Consider extracting transport-neutral health and activation contracts into a package with no Durable execution-stack dependency. **Why:** `ForgeTrust.AppSurface.Durable.Provider` intentionally references the full Durable package today; a transport host therefore receives Flow, Core, and Workers transitively even if it only activates a runtime. **Pros:** Could reduce restore/version coupling for independently deployed activators. **Cons:** Adds another public package, coordinated-version policy, API baselines, and migration work before any adopter has shown the current graph is a problem. **Context:** The executable-contract adoption rail keeps the existing Provider boundary and enforces that public activation contracts expose no PostgreSQL, ASP.NET Core, or application types. **Depends on / blocked by:** An independent activator that measures the transitive graph as an adoption or deployment blocker.
- **What:** Consider a dedicated PostgreSQL heartbeat-maintenance principal and scheduler instead of runtime-owned opportunistic pruning. **Why:** Migration 0005 deliberately trusts the isolated runtime credential for all unscoped heartbeat rows; the planned retention function extends that principal with bounded stale-row deletion. **Pros:** Narrows deletion capability if a deployment cannot accept the existing runtime-principal blast radius. **Cons:** Adds a credential, rotation path, scheduler owner, deployment ordering, and failure mode for capacity-only maintenance. **Context:** The current design treats caller-supplied worker identity as a correctness guard, not authentication, and constrains deletion by stale cutoff, table, signature, advisory lock, and batch size. **Depends on / blocked by:** A concrete adopter threat model that cannot treat the runtime credential as a trusted maintenance principal.
...
## Logical configuration-key follow-ups
