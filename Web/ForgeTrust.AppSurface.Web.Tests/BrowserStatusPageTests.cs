...
using ForgeTrust.AppSurface.Web.Tests.SharedErrorPagesFixture;
...
public sealed class BrowserStatusPageTests
{
    ...
    {
        { StatusCodes.Status401Unauthorized, BrowserStatusPageDefaults.ReservedUnauthorizedRoute, "AppSurface default 401" },
        { StatusCodes.Status403Forbidden, BrowserStatusPageDefaults.ReservedForbiddenRoute, "AppSurface default 403" },
        { StatusCodes.Status404NotFound, BrowserStatusPageDefaults.ReservedNotFoundRoute, "AppSurface default 404" }
    };

    public static TheoryData<int, HttpStatusCode, string> SupportedStatusResponses => new()
    {
        { StatusCodes.Status401Unauthorized, HttpStatusCode.Unauthorized, "AppSurface default 401" },
        { StatusCodes.Status403Forbidden, HttpStatusCode.Forbidden, "AppSurface default 403" },
        { StatusCodes.Status404NotFound, HttpStatusCode.NotFound, "AppSurface default 404" }
    };
    ...
    public async Task EnabledMode_UpgradesControllersOnlyApps_AndMapsReservedRoutes()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Contains("AppSurface default 401", html);
    }
    ...
    public async Task DirectRequests_ToReserved500_Return404WithoutRenderingFallback()
    {
        ...
        using var response = await runningApp.Client.GetAsync("/_appsurface/errors/500");
        ...
        string expectedMarker)
    {
        ...
        Assert.True(response.Headers.Contains($"X-AppSurface-Reexecuted-{statusCode}"));
    }
    ...
    public async Task HtmlHeadRequests_ToEmptySupportedStatuses_ReExecuteAndPreserveStatus()
    {
        ...
        Assert.True(response.Headers.Contains("X-AppSurface-Reexecuted-403"));
    }
        ...
        string expectedMarker)
    {
        ...
        Assert.False(response.Headers.Contains($"X-AppSurface-Reexecuted-{statusCode}"));
    }
    ...
    public async Task SharedRclView_IsUsed_WhenAppOverrideIsMissing(int statusCode, string expectedMarker)
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        Assert.Contains(expectedMarker, html);
        Assert.DoesNotContain($"AppSurface default {statusCode}", html);
    }
    ...
    public void ValidateConfiguredViews_ThrowsWithStatusAndSearchedLocations_WhenNoViewsResolve()
    {
        var viewEngine = new StubCompositeViewEngine(
            ...
            defaultFrameworkResult: ViewEngineResult.NotFound(
                ...
                ["/Views/_AppSurface/Errors/StatusPage.cshtml"]));
        ...
        Assert.Contains("/Views/_AppSurface/Errors/StatusPage.cshtml", exception.Message);
    ...
    private static async Task<RunningAppHandle> StartHostAsync<TModule>(
        ...
        where TModule : class, IAppSurfaceWebModule, new()
    {
    ...
    private abstract class BaseTestWebModule : IAppSurfaceWebModule
    {
        ...
        public override void ConfigureWebApplication(StartupContext context, IApplicationBuilder app)
        {
            app.Use(
                async (httpContext, next) =>
                {
                    ...
                    {
                        httpContext.Response.Headers[$"X-AppSurface-Reexecuted-{reExecuteFeature.OriginalStatusCode}"] = "true";
                    }
