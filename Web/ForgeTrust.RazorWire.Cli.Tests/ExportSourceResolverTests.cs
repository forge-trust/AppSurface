...
public class ExportSourceResolverTests
{
    ...
    [Fact]
    public async Task ResolveAsync_Should_Surface_Strict_AppSurfaceDocs_Harvest_Summary_When_Process_Exits_Early()
    {
        var fakeProcess = new FakeTargetAppProcess();
        var factory = new FakeTargetAppProcessFactory(_ => fakeProcess);
        var resolver = CreateResolver(
        ...
        fakeProcess.OnStart = () =>
        {
            ...
            fakeProcess.EmitError(
                "AppSurfaceDocs strict harvest failed because every configured harvester failed, timed out, or canceled. Diagnostics: appsurfacedocs.harvest.all_failed (Critical): All AppSurfaceDocs harvesters failed.");
            fakeProcess.TriggerExit();
        };

        var ex = await Assert.ThrowsAsync<InvalidOperationException>(async () => await resolver.ResolveAsync(request));

        Assert.Contains("AppSurfaceDocs strict harvest failed", ex.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("appsurfacedocs.harvest.all_failed", ex.Message, StringComparison.Ordinal);
    ...
    public async Task ResolveLaunchRequestAsync_Should_Publish_Project_When_NoBuild_Is_False()
    {
        ...
        var expectedPath = Path.GetFullPath(Path.Combine(tempDir.FullPath, "bin", "razorwire-export", "MySite.dll"));
        ...
        Assert.Contains(Path.Combine(tempDir.FullPath, "bin", "razorwire-export"), capturedPublishArgs);
    }
    ...
    public async Task ResolveLaunchRequestAsync_Should_Inject_Framework_Arg_When_Framework_Specified()
    {
        ...
        var expectedPath = Path.Combine(tempDir.FullPath, "bin", "razorwire-export", "MySite.dll");
        IReadOnlyList<string>? capturedPublishArgs = null;
        A.CallTo(() => mockRazorWireExecutor.ExecuteCommandAsync(
                A<string>._,
                A<IReadOnlyList<string>>._,
        ...
        var normalizedExpected = ExportSourceResolver.NormalizePathForMacOS(
            Path.Combine(tempDir.FullPath, "bin", "razorwire-export", "MySite.dll"));
