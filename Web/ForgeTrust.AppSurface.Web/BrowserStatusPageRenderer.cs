...
/// <c>~/Views/Shared/403.cshtml</c>. If that view is missing, the renderer falls back to AppSurface's shared
...
internal sealed class BrowserStatusPageRenderer
{
    ...
    public async Task RenderAsync(HttpContext httpContext)
    {
        ...
        if (!BrowserStatusPageDescriptor.TryGet(model.StatusCode, out var descriptor))
        {
            throw new InvalidOperationException(
                $"AppSurface browser status pages do not support HTTP status {model.StatusCode}. Use one of the supported browser status page codes before rendering.");
        }
    ...
    private string ResolveViewPathUncached(BrowserStatusPageDescriptor descriptor)
    {
        ...
        throw new InvalidOperationException(
            $"AppSurface browser status pages are enabled for {descriptor.StatusCode}, but no Razor view could be resolved. Add '{descriptor.AppViewPath}' or restore the framework fallback view '{descriptor.FrameworkFallbackViewPath}'.{Environment.NewLine}Searched locations:{Environment.NewLine}{locationsMessage}");
    }
