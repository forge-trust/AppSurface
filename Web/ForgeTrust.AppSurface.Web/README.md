...
| `ASWEBTHEME008` | A selected or fallback pair is unsafe. | The policy returned a default, empty, or unregistered id, or the resolver's configured default was unsafe, absent from the sealed registry, or had different roles. | Return `false` for the registered configured default, return one id from the sealed registry, and keep the resolver default's pair id and roles aligned with it. | [Guide](#host-owned-theme-selection) |
