...
public class TextMateSharpAppSurfaceDocsCodeHighlighterTests
{
    private readonly TextMateSharpAppSurfaceDocsCodeHighlighter _highlighter = new(
        new AppSurfaceDocsCodeLanguageCatalog(),
        NullLogger<TextMateSharpAppSurfaceDocsCodeHighlighter>.Instance);

    [Fact]
    public void Highlight_ShouldRenderCSharpWithTokenSpans()
    {
        var result = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(
            "public sealed class Demo { public string Name { get; init; } = \"AppSurfaceDocs\"; }",
            "cs"));
        ...
        Assert.Contains("AppSurfaceDocs", result.Html);
    }
    ...
    public void Highlight_ShouldKeepLanguageLabelOutOfRenderedCodeText(
        ...
        string code)
    {
        var result = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(code, language));
    ...
    {
        var result = _highlighter.Highlight(new AppSurfaceDocsCodeBlock("plain <b>code</b>", null));
    ...
    {
        var result = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(null, "plaintext"));
    ...
    {
        var code = new string('x', TextMateSharpAppSurfaceDocsCodeHighlighter.MaxHighlightedCodeBlockCharacters + 1);

        var result = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(code, "csharp"));
    ...
    {
        var highlighter = new TextMateSharpAppSurfaceDocsCodeHighlighter(
            new AppSurfaceDocsCodeLanguageCatalog(),
            NullLogger<TextMateSharpAppSurfaceDocsCodeHighlighter>.Instance,
            _ => null);

        var result = highlighter.Highlight(new AppSurfaceDocsCodeBlock("@page", "csharp"));
    ...
    {
        var code = string.Join('\r', Enumerable.Repeat("x", TextMateSharpAppSurfaceDocsCodeHighlighter.MaxHighlightedCodeBlockLines + 1));
    ...
    {
        var atLimit = string.Join("\r\n", Enumerable.Repeat("x", TextMateSharpAppSurfaceDocsCodeHighlighter.MaxHighlightedCodeBlockLines));
        var atLimitResult = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(atLimit, "csharp"));

        Assert.True(atLimitResult.IsHighlighted);
        Assert.Equal("csharp", atLimitResult.NormalizedLanguage);
        Assert.Contains("x\nx", atLimitResult.Html);

        var overLimit = string.Join("\r\n", Enumerable.Repeat("x", TextMateSharpAppSurfaceDocsCodeHighlighter.MaxHighlightedCodeBlockLines + 1));
        var overLimitResult = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(overLimit, "csharp"));
    ...
    {
        _highlighter.Highlight(new AppSurfaceDocsCodeBlock("public class One { }", "csharp"));
        _highlighter.Highlight(new AppSurfaceDocsCodeBlock("public class Two { }", "cs"));
    ...
    {
        var language = new AppSurfaceDocsCodeLanguage(
            "custom",
            "custom",
            "Custom",
    ...
    public void AppendTokens_ShouldEscapeUntokenizedLines()
    {
        ...
        TextMateSharpAppSurfaceDocsCodeHighlighter.AppendTokens(builder, "<empty>", []);
    ...
    public void AppendTokens_ShouldPreserveGapsAndTrailingText()
    {
        ...
        TextMateSharpAppSurfaceDocsCodeHighlighter.AppendTokens(builder, "ab<cde>fg&", tokens);
    ...
    {
        Assert.Equal(expected, TextMateSharpAppSurfaceDocsCodeHighlighter.ResolveTokenClass([scope]));
    }
