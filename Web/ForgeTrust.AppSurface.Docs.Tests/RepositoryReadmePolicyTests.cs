...
public sealed partial class RepositoryReadmePolicyTests
{
    ...
    public void PublicPackageReadmes_ShouldLinkToCurrentReleaseCandidate()
    {
        ...
        Assert.Equal(15, requiredReadmes.Length);
        ...
        foreach (var readmePath in requiredReadmes)
        {
            ...
            var expectedTarget = Path.GetFullPath(Path.Join("releases", "v0.1.0-rc.2.md"), repoRoot);
            ...
            Assert.True(linksToReleaseNote, $"{readmePath} must link to the v0.1.0 RC 2 release note.");
        }
    }
