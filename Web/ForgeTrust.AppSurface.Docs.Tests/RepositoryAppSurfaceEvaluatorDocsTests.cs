...
public sealed class RepositoryAppSurfaceEvaluatorDocsTests
{
    ...
    {
        var docs = await HarvestRepositoryDocsAsync();
        var releaseNote = SingleDoc(docs, "releases/v0.1.0-rc.1.md");

        Assert.Equal("AppSurface 0.1.0 RC 1", releaseNote.Metadata?.Title);
        Assert.Equal("release-note", releaseNote.Metadata?.PageType);
        Assert.Equal("Releases", releaseNote.Metadata?.NavGroup);
        Assert.Equal(["Releases", "v0.1.0-rc.1"], releaseNote.Metadata?.Breadcrumbs);
        Assert.Equal(["releases/v0.1-preview", "releases/v0.1-preview.md", "releases/v0.1-preview.md.html"], releaseNote.Metadata?.RedirectAliases);
        ...
        var aggregator = CreateAggregator(docs);
        var details = await aggregator.GetDocDetailsAsync("releases/v0.1.0-rc.1.md");
        ...
        var searchDocument = Assert.Single(
            ...
            document => string.Equals(document.SourcePath, "releases/v0.1.0-rc.1.md", StringComparison.OrdinalIgnoreCase));

        Assert.Equal("/docs/releases/v0.1.0-rc.1", searchDocument.Path);
