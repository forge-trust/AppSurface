...
public sealed class RepositoryAppSurfaceEvaluatorDocsTests
{
    ...
    {
        var docs = await HarvestRepositoryDocsAsync();
        var releaseNote = SingleDoc(docs, "releases/v0.1.0-rc.4.md");

        Assert.Equal("Release 0.1.0-rc.4", releaseNote.Metadata?.Title);
        Assert.Equal("release-note", releaseNote.Metadata?.PageType);
        Assert.Equal("Releases", releaseNote.Metadata?.NavGroup);
        Assert.Equal(["Releases", "v0.1.0-rc.4"], releaseNote.Metadata?.Breadcrumbs);
        ...
        var aggregator = CreateAggregator(docs);
        var details = await aggregator.GetDocDetailsAsync("releases/v0.1.0-rc.4.md");
        ...
        var searchDocument = Assert.Single(
            ...
            document => string.Equals(document.SourcePath, "releases/v0.1.0-rc.4.md", StringComparison.OrdinalIgnoreCase));

        Assert.Equal("/docs/releases/v0.1.0-rc.4", searchDocument.Path);
