...
Native `EventSource` does not expose failed HTTP response bodies or status codes to application JavaScript. RazorWire dispatches `razorwire:stream:error` from registered `rw-stream-source` elements when the browser reports a stream error. Use that event for client-side diagnostics, and use the browser Network tab plus server log event `13701 StreamAdmissionRejected` to distinguish `400`, `403`, and `429` during development.
