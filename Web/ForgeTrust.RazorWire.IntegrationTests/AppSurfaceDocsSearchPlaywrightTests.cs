...
public sealed class AppSurfaceDocsIntegrationCollection : ICollectionFixture<AppSurfaceDocsPlaywrightFixture>
{
    public const string Name = "AppSurfaceDocsIntegrationCollection";
}

[Collection(AppSurfaceDocsIntegrationCollection.Name)]
[Trait("Category", "Integration")]
public sealed class AppSurfaceDocsSearchPlaywrightTests
{
    ...
    public async Task SearchPage_NoResultsRecovery_ShowsSummaryAndClearsFilters()
    {
        ...
        await page.GotoAsync($"{_fixture.DocsUrl}/search?q=no-such-query&pageType=guide&component=AppSurface%20Docs&audience=developer&status=stable");
        await WaitForSearchPageSettledAsync(page);

        await ExpectVisibleTextAsync(
            page,
            "#docs-search-page-results",
            "Current search: query \"no-such-query\", Page Type: Guide, Component: AppSurface Docs, Audience: Developer, Status: Stable.");
...
public sealed class AppSurfaceDocsPlaywrightFixture : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            _appHost = await AppSurfaceDocsInProcessHost.StartAsync("http://127.0.0.1:0");
    ...
    private async Task WaitForAppReadyAsync(string docsUrl, TimeSpan timeout)
    {
        ...
        await PollUntilAsync(
            timeout,
            timeoutMessage: "AppSurface Docs app did not become ready",
            appExitedMessage: "AppSurface Docs app exited before it became ready",
    ...
    private async Task<string> WarmSearchIndexAndResolveQueryAsync(string baseUrl, TimeSpan timeout)
    {
        ...
        await PollUntilAsync(
            timeout,
            timeoutMessage: "AppSurface Docs search index did not become ready",
            appExitedMessage: "AppSurface Docs app exited before search index warmed",
            async cancellationToken =>
            {
