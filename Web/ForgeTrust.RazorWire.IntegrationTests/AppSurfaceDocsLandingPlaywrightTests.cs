...
public sealed class AppSurfaceDocsLandingPlaywrightTests
{
        ...
        string badge)
    {
        ...
        var ariaLabel = await card.GetAttributeAsync("aria-label");
        Assert.NotNull(ariaLabel);
        Assert.StartsWith("View ", ariaLabel, StringComparison.Ordinal);
        Assert.Contains(title, ariaLabel, StringComparison.OrdinalIgnoreCase);
