...
- The bundled RazorDocs UI includes its generated stylesheet and docs runtime files as static web assets and assembly-embedded fallback resources. The layout resolves the correct stylesheet path automatically from the host's root module shape for standalone/root-module hosts versus embedded application-part consumers, and it renders RazorDocs-owned CSS and JavaScript URLs with content-derived `v` query strings. Those version keys are computed from the embedded package assets so route-local aliases such as `/docs/search.css` and `/docs/search-client.js` are cache-busted even when ASP.NET Core cannot resolve them as direct static-web-asset paths. Keep new RazorDocs-owned chrome assets on the `RazorDocsAssetVersioner` path; plain `asp-append-version` is not enough for endpoint aliases or legacy redirects.
