...
public class AppSurfaceDocsWebModule : IAppSurfaceWebModule
{
        ...
        Action<IEndpointConventionBuilder> applyInstanceConventions)
    {
        ...
        MapNamedBrandingAssetDirectory(endpoints, options, applyInstanceConventions);
        MapNamedLegacyAssetRedirect(
            endpoints,
        ...
        var namedHarvest = MapNamedControllerRoute(
            ...
            "Harvest",
            applyInstanceConventions);
        ApplyNamedPolicy(namedHarvest, ResolveHarvestReadAuthorizationPolicyName(options, endpoints.ServiceProvider));
        applyInstanceConventions(endpoints.MapMethods(
            harvestRebuild,
            [HttpMethods.Delete, HttpMethods.Get, HttpMethods.Head, HttpMethods.Options, HttpMethods.Patch, HttpMethods.Put],
            RejectHarvestRebuildUnsupportedMethodAsync));
        ...
        ApplyNamedPolicy(namedHealth, healthPolicy);
        ApplyNamedPolicy(namedHealthJson, healthPolicy);
        ...
        ApplyNamedPolicy(namedRouteInspector, routeInspectorPolicy);
        ApplyNamedPolicy(namedRouteInspectorJson, routeInspectorPolicy);
        ...
        {
            namedMarkdownUnsupportedMethods.RequireAuthorization(options.MarkdownDownload.AuthorizationPolicy!);
        }
    ...
    private static void ApplyNamedPolicy(IEndpointConventionBuilder endpoint, string? policyName)
    {
    ...
    private static void MapNamedLegacyAssetRedirect(
        IEndpointRouteBuilder endpoints,
        string route,
        string targetPath,
        Action<IEndpointConventionBuilder> applyInstanceConventions)
    ...
    private static void MapNamedBrandingAssetDirectory(
        IEndpointRouteBuilder endpoints,
