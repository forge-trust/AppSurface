...
- For published version catalogs, keep the standard `catalog.json` beside `releases/` layout when possible and leave `AppSurfaceDocs:Versioning:TrustedReleaseRootPath` unset. If your release store lives elsewhere, configure `TrustedReleaseRootPath` once and keep every catalog `exactTreePath` relative to that directory.
...
- Treat the trusted release root as an operator-owned immutable static export store. Symlinks, junctions, reparse points, rooted catalog paths, and `../` escapes are unavailable and are never mounted.
