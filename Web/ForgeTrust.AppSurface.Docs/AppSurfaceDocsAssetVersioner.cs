...
/// </summary>
/// <remarks>
/// AppSurface Docs often exposes package assets through friendly route-local aliases such as
...
/// surface should render. Use it for AppSurface Docs-owned CSS and JavaScript assets that must survive browser, CDN, or
/// service-worker caches across deployments. Do not use it for reader-authored document links or external CDN URLs.
/// </remarks>
internal sealed class AppSurfaceDocsAssetVersioner
{
    ...
    /// </summary>
    /// <param name="docsUrlBuilder">The URL builder for the active AppSurface Docs surface.</param>
    ...
    /// <param name="embeddedAssetPath">The asset path under the AppSurface Docs embedded asset root.</param>
    /// <returns>
    ...
    private static string? ComputeVersion(string normalizedEmbeddedAssetPath)
    {
        ...
        using var stream = AppSurfaceDocsAssembly.GetManifestResourceStream(resourceName);
        if (stream is null)
        {
            return null;
        }
