...
public class CSharpDocHarvester : IDocHarvester, IDocHarvesterDiagnosticProvider
{
        ...
        List<DocHarvestDiagnostic> diagnostics)
    {
        ...
        foreach (var typeDeclaration in root.DescendantNodes().OfType<TypeDeclarationSyntax>())
        {
            ...
            var pythonModulePath = TryGetPythonModulePath(typeDeclaration, relativePath, diagnostics);
            ...
            namespacePage.Types.Add(
                new CSharpTypeDocument(
                    ...
                    methodGroups,
                    typedProperties,
                    PythonModulePaths: pythonModulePath is null ? null : [pythonModulePath]));
            stubs.Add(
                new DocNode(
                    GetDisplayTypeName(typeDeclaration),
                ...
                group =>
                {
                    ...
                    return first with
                    {
                        ...
                        Properties = group
                            ...
                            .ToArray(),
                        PythonModulePaths = group
                            .SelectMany(type => type.PythonModulePaths ?? [])
                            .ToArray()
                    };
                })
            .ToArray();
