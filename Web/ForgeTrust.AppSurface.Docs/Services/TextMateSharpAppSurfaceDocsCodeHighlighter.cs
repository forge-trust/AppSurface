...
/// </summary>
internal sealed class TextMateSharpAppSurfaceDocsCodeHighlighter : IAppSurfaceDocsCodeHighlighter
{
    ...
    private readonly AppSurfaceDocsCodeLanguageCatalog _languageCatalog;
    private readonly ILogger<TextMateSharpAppSurfaceDocsCodeHighlighter> _logger;
    private readonly RegistryOptions _registryOptions;
    private readonly Registry _registry;
    private readonly Func<AppSurfaceDocsCodeLanguage, IGrammar?>? _loadGrammarOverride;
    ...
    public TextMateSharpAppSurfaceDocsCodeHighlighter(
        AppSurfaceDocsCodeLanguageCatalog languageCatalog,
        ILogger<TextMateSharpAppSurfaceDocsCodeHighlighter> logger)
    {
    ...
    internal TextMateSharpAppSurfaceDocsCodeHighlighter(
        AppSurfaceDocsCodeLanguageCatalog languageCatalog,
        ILogger<TextMateSharpAppSurfaceDocsCodeHighlighter> logger,
        Func<AppSurfaceDocsCodeLanguage, IGrammar?> loadGrammarOverride)
    ...
    public AppSurfaceDocsHighlightedCode Highlight(AppSurfaceDocsCodeBlock block)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Falling back to escaped plaintext for AppSurface Docs code block language {Language}.",
    ...
    internal IGrammar? LoadGrammar(AppSurfaceDocsCodeLanguage language)
    {
            ...
            textMateLanguageId => new Lazy<IGrammar?>(
                () =>
                {
                    ...
                    catch (Exception ex)
                    {
                        _logger.LogWarning(
                            ex,
                            "Failed to load TextMate grammar for AppSurface Docs language {Language}.",
                            language.NormalizedLanguage);
                        return null;
                    }
                },
    ...
    private AppSurfaceDocsHighlightedCode RenderHighlighted(
        string code,
        AppSurfaceDocsCodeLanguage language,
        IGrammar grammar)
    {
        ...
        return new AppSurfaceDocsHighlightedCode(builder.ToString(), language.NormalizedLanguage, IsHighlighted: true);
    }
    ...
    /// <returns>The AppSurface Docs token modifier, or <see langword="null"/> for unstyled scopes.</returns>
    ...
    private static AppSurfaceDocsHighlightedCode RenderPlain(string code, AppSurfaceDocsCodeLanguage language)
    {
        ...
        AppendWrapperClose(builder);
        return new AppSurfaceDocsHighlightedCode(builder.ToString(), language.NormalizedLanguage, IsHighlighted: false);
    }

    private static void AppendWrapperOpen(StringBuilder builder, AppSurfaceDocsCodeLanguage language, bool isHighlighted)
    {
        builder
