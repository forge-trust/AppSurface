...
internal static class DocMetadataFactory
{
    ...
    internal static string? DeriveComponentFromPath(string path)
    {
        ...
        {
            if (segment.StartsWith(AppSurfaceNamespacePrefix, StringComparison.OrdinalIgnoreCase))
            {
                return DeriveComponentFromNamespace(segment);
            }
        }

        return path.Contains("AppSurface", StringComparison.OrdinalIgnoreCase) ? "AppSurface" : null;
    }
    ...
    internal static string? DeriveComponentFromNamespace(string namespaceName)
    {
        ...
        {
            var suffix = namespaceName[AppSurfaceNamespacePrefix.Length..];
            var parts = suffix.Split('.', StringSplitOptions.RemoveEmptyEntries);
            return parts.Length == 0 ? "AppSurface" : GetAppSurfaceComponentName(parts);
        }

        if (namespaceName.Equals(RazorWireNamespacePrefix, StringComparison.OrdinalIgnoreCase)
            || namespaceName.StartsWith(RazorWireNamespacePrefix + ".", StringComparison.OrdinalIgnoreCase))
        {
            return "RazorWire";
        }
    ...
    private static string GetAppSurfaceComponentName(IReadOnlyList<string> parts)
    {
        if (parts.Count == 1)
        {
            return parts[0];
