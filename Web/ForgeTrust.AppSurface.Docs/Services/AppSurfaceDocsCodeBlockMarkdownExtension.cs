...
/// </summary>
internal sealed class AppSurfaceDocsCodeBlockMarkdownExtension : IMarkdownExtension
{
    private readonly IAppSurfaceDocsCodeHighlighter _highlighter;
    ...
    internal AppSurfaceDocsCodeBlockMarkdownExtension(IAppSurfaceDocsCodeHighlighter highlighter)
    {
    ...
    public void Setup(MarkdownPipeline pipeline, IMarkdownRenderer renderer)
    {
        if (renderer is HtmlRenderer htmlRenderer)
        {
            htmlRenderer.ObjectRenderers.ReplaceOrAdd<CodeBlockRenderer>(
                new AppSurfaceDocsCodeBlockRenderer(_highlighter));
        }
    }
    ...
    /// <returns>The default AppSurface Docs code highlighter.</returns>
    internal static IAppSurfaceDocsCodeHighlighter CreateDefaultHighlighter(
        ILogger<TextMateSharpAppSurfaceDocsCodeHighlighter> logger)
    {
        ArgumentNullException.ThrowIfNull(logger);

        return new TextMateSharpAppSurfaceDocsCodeHighlighter(
            AppSurfaceDocsCodeLanguageCatalog.Shared,
            logger);
    }
}
...
/// </summary>
internal sealed class AppSurfaceDocsCodeBlockRenderer : HtmlObjectRenderer<CodeBlock>
{
    private readonly IAppSurfaceDocsCodeHighlighter _highlighter;
    ...
    internal AppSurfaceDocsCodeBlockRenderer(IAppSurfaceDocsCodeHighlighter highlighter)
    {
        ArgumentNullException.ThrowIfNull(highlighter);
        _highlighter = highlighter;
    }
    ...
    protected override void Write(HtmlRenderer renderer, CodeBlock obj)
    {
        ...
        var language = obj is FencedCodeBlock fencedCodeBlock
            ...
            : null;
        var highlighted = _highlighter.Highlight(new AppSurfaceDocsCodeBlock(code, language));
