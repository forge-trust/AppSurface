...
(() => {
  ...
  function getSearchPageElements() {
    ...
    return {
      root,
      input: document.getElementById('docs-search-page-input') as HTMLInputElement | null,
      status: document.getElementById('docs-search-page-status'),
      filtersToggle: document.getElementById('docs-search-page-filters-toggle') as HTMLButtonElement | null,
      ...
      suggestionButtons: root
        ? Array.from(root.querySelectorAll<HTMLElement>('[data-rw-search-suggestion]'))
        : []
    };
  }
  ...
  function installDocsPartialHook() {
    ...
    document.addEventListener('turbo:before-fetch-request', (event) => {
      const turboEvent = event as Event & { detail?: { fetchOptions?: { headers?: any }; url?: URL }; target: EventTarget | null };
      const targetFrame = turboEvent.target instanceof HTMLElement ? turboEvent.target : null;
      const turboFrameHeader = getHeader(turboEvent.detail?.fetchOptions?.headers, 'Turbo-Frame');
      const isDocFrameRequest = (targetFrame && targetFrame.id === docsFrameId) || turboFrameHeader === docsFrameId;
      if (!isDocFrameRequest) {
        return;
      }

      const requestUrl = turboEvent.detail?.url;
  ...
  function renderSearchLifecycleBadges(item, className = '') {
    return getSearchLifecycleBadges(item, className)
      .map(({ label, className: badgeClassName }) => `<span class="${badgeClassName}">${escapeHtml(label)}</span>`)
      .join('');
  }

  function appendSearchLifecycleBadges(parent, item, className = '') {
    getSearchLifecycleBadges(item, className).forEach(({ label, className: badgeClassName }) => {
      parent.append(createElement('span', badgeClassName, label));
    });
  }

  function getSearchLifecycleBadges(item, className = '') {
    ...
    return [
      lifecycle ? { label: lifecycle, className: `${classes} docs-search-result-badge-lifecycle` } : null,
      deprecated ? { label: deprecated, className: `${classes} docs-search-result-badge-deprecated` } : null
    ].filter(Boolean);
  }
  ...
  function createElement(tagName, className = '', textContent = undefined) {
  ...
  function createSearchResultArticle(doc, queryTokens, options: any = {}) {
    ...
    appendSearchLifecycleBadges(badgeRow, doc);
  ...
  function ensureMiniSearchLoaded() {
    ...
    if (!searchData.runtimePromise) {
      const runtimeScripts = Array.from(
        document.querySelectorAll<HTMLScriptElement>('script[data-rw-search-runtime="minisearch"]:not([data-rw-search-failed="true"])')
      );
  ...
  function renderSidebarResults(sidebar, queryResults, query, activeIndex = -1) {
    ...
    results.innerHTML = safeQueryResults.map((item, index) => {
      ...
      return `<li id="docs-search-option-${index}" role="option" aria-selected="${selected}" tabindex="-1" class="docs-search-option" data-href="${escapeHtml(item.path)}">
        <a href="${escapeHtml(item.path)}"${navigationAttributes} aria-label="${escapeHtml(createSearchResultLinkLabel(item))}" data-rw-product-result-rank="${index + 1}" data-rw-product-result-kind="${escapeHtml(resultKind)}" data-rw-product-result-surface="sidebar">
  ...
  function bindSearchPage() {
    ...
    filters.addEventListener('click', (event) => {
      ...
      const chip = target.closest('[data-rw-facet-key][data-rw-facet-value]');
    ...
    activeFilters.addEventListener('click', (event) => {
      ...
      const button = target.closest('[data-rw-clear-facet-key]');
    ...
    if (mobileFilterMedia && !window[mobileFilterListenerBoundFlag]) {
      ...
      window[mobileFilterListenerBoundFlag] = true;
    }
