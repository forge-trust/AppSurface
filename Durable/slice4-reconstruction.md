...
| 6 | Flow store integration verified against PostgreSQL. | `PostgreSql.Tests/PostgreSqlDurableFlowStoreTests.cs` | adapted | Flow store integration tests | Tests recovery invariants, single-use event delivery, timer expiry with a forced post-commit child-process kill, child activity completion/cancellation, and RLS isolation across current minimum compatibility floor. |
