...
| 10 | Security recipe grants least privilege for Flow tables. | `configure-postgresql-roles.sql` | adapted | Role configuration script | Updates permissions for runtime role on `flow_instance`, `flow_command`, `flow_history`, `flow_wait`, and `flow_timer`. |
