...
but the current provider schema must be applied through `0011_runtime_heartbeat_retention.sql`; child Work discovery
...
1. **Schema Deployment**: Migration owner checks status, applies the current reviewed forward migrations (including `0001_work_shared.sql`, `0002_forced_rls.sql`, `0003_flow_protocol.sql`,
   `0009_work_contract_discovery.sql`, `0010_runtime_health_observation.sql`, and `0011_runtime_heartbeat_retention.sql`), reads `StoreId`,
...
2. Call `GetStatusAsync`, `ApplyAsync` (applying current reviewed forward migrations through `0011_runtime_heartbeat_retention.sql`; Slice 4 exercises Flow facts introduced through `0003_flow_protocol.sql`), and `InitializeRuntimeEpochAsync`; capture StoreId and active epoch.
