...
The CLI `apply` path is appropriate only if the measured 0011 index build fits its 330-second migration-command deadline and the full operation, including any earlier pending migrations, fits 45 minutes. For a larger table, use the reviewed generated script through a client with a deliberately sized operation deadline and `ON_ERROR_STOP=1`; keep the script's package advisory lock and transaction wrapper intact. Do not execute the embedded migration fragment directly. Do not run `apply` and the generated script concurrently. The same SQL migration must be recorded once in `schema_migration`.
