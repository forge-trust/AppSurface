...
internal sealed class PostgreSqlDurableWorkStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var fingerprint = DurableWorkRequestFingerprint.Compute(request);
        ...
        try
        {
            ...
            await using (var reader = await claimCommand.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                claim = new PostgreSqlDurableWorkClaim(
                    ...
                    reader.GetFieldValue<TimeSpan>(19));
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        return await reader.ReadAsync(cancellationToken).ConfigureAwait(false)
            ? new PostgreSqlWorkClaimTransition(ParseWorkState(reader.GetString(0)), reader.GetString(1))
            : null;
    }
        ...
        try
        {
            ...
            const string workSql = """
                ...
                WHERE work.scope_id = @scope_id
                  ...
                  AND scope.state = 'active'
                  AND scope.generation = work.scope_generation
                RETURNING work.provider_key, work.revision, work.lease_expires_at,
                          work.cancellation_requested_at IS NOT NULL;
                """;
            ...
            const string permitSql = """
                INSERT INTO appsurface_durable.effect_permit
                    (permit_id, scope_id, work_id, attempt_number, lease_generation, scope_generation,
                     runtime_epoch, provider_key, status)
                VALUES
                    (@permit_id, @scope_id, @work_id, @attempt_number, @lease_generation, @scope_generation,
                     @runtime_epoch, @provider_key, 'granted')
            ...
            permitCommand.Parameters.AddWithValue("provider_key", providerKey);
    ...
    {
        const string sql = """
            SELECT permit.permit_id, permit.provider_key, permit.permitted_at, work.revision, work.lease_expires_at,
                   work.cancellation_requested_at IS NOT NULL
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (completion.RetryDelay is { } retryDelay && retryDelay <= TimeSpan.Zero)
        {
            throw new ArgumentOutOfRangeException(nameof(completion), "Provider Retry-After must be positive.");
        }
                ...
                (
                    SELECT
                        ...
                        clock_timestamp() + LEAST(
                            COALESCE(@retry_delay, work.initial_retry_delay),
                            work.retry_after_cap) AS next_due_at
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await using (var dispatchCommand = new NpgsqlCommand(dispatchSql, connection, transaction))
            {
                ...
                await dispatchCommand.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
            }
        ...
        CancellationToken cancellationToken)
    {
        const string sql = """
            ...
            (
                ...
                SET suspended_from_state = state,
                    suspended_from_terminal_code = terminal_code,
                    state = 'suspended',
                    terminal_code = @scope_disabled_code,
                    lease_owner = NULL,
                    lease_started_at = NULL,
                    lease_expires_at = NULL,
                    revision = revision + 1,
                ...
                RETURNING flow_instance_id, revision, current_node_id, authoring_model,
                          command_schema_version, definition_fingerprint
            ),
            canceled_waits AS
            (
                UPDATE appsurface_durable.flow_wait AS wait
                SET state = 'canceled',
                    resolved_revision = suspended_flows.revision,
                    resolved_at = clock_timestamp()
                FROM suspended_flows
                WHERE wait.scope_id = @scope_id
                  AND wait.flow_instance_id = suspended_flows.flow_instance_id
            ...
            (
                ...
                WHERE timer.scope_id = @scope_id
                  AND timer.flow_instance_id = suspended_flows.flow_instance_id
                  AND timer.state = 'scheduled'
                RETURNING timer.timer_id
            ),
            historied_flows AS
            (
                INSERT INTO appsurface_durable.flow_history
                    (scope_id, flow_instance_id, aggregate_revision, event_type,
                     authoring_model, command_schema_version, definition_fingerprint, actor_id, reason_code,
                     node_id, details)
                SELECT @scope_id, flow_instance_id, revision, 'scope_disabled',
                       authoring_model, command_schema_version, definition_fingerprint, @actor_id, @reason_code,
                       current_node_id, jsonb_build_object('code', @scope_disabled_code)
                FROM suspended_flows
                RETURNING event_id
            ),
            suspended_schedules AS
            (
                ...
                RETURNING schedule_id, revision, generation
            ),
            invalidated_occurrences AS
            (
                UPDATE appsurface_durable.schedule_occurrence AS occurrence
                SET state = 'invalidated',
                    resolved_at = clock_timestamp()
                FROM suspended_schedules
                WHERE occurrence.scope_id = @scope_id
                  AND occurrence.schedule_id = suspended_schedules.schedule_id
                  AND occurrence.state IN ('ready', 'queued', 'coalesced')
                RETURNING occurrence.occurrence_id
            ),
            historied_schedules AS
            (
                INSERT INTO appsurface_durable.schedule_history
                    (scope_id, schedule_id, aggregate_revision, schedule_generation, event_type,
                     actor_id, reason_code, details)
                SELECT @scope_id, schedule_id, revision, generation, 'scope_disabled',
                       @actor_id, @reason_code, jsonb_build_object('code', @scope_disabled_code)
                FROM suspended_schedules
                RETURNING event_id
            )
            UPDATE appsurface_durable.dispatch
        ...
        const string sql = """
            ...
            (
                ...
                (
                    ...
                    payload_classification, payload_retention, request_sha256,
                    state, provider_safety, provider_key, due_at, scope_generation, runtime_epoch,
                    maximum_attempts, maximum_elapsed, backoff_algorithm, backoff_version, jitter_seed,
                    initial_retry_delay, retry_after_cap, lease_duration, lease_renewal_cadence,
                ...
                (
                    ...
                    @contract_id, @payload_schema_version, @codec_id, @payload, @payload_sha256,
                    @payload_classification, @payload_retention, @request_sha256,
                    'pending', @provider_safety, @provider_key, COALESCE(@due_at, clock_timestamp()),
                    @scope_generation, @runtime_epoch,
                    @maximum_attempts, @maximum_elapsed, @backoff_algorithm, 1, @jitter_seed,
                    @initial_retry_delay, @retry_after_cap, @lease_duration, @lease_renewal_cadence,
                    @maximum_lease_lifetime
                )
        ...
        byte[] fingerprint)
    {
        ...
        command.Parameters.AddWithValue("provider_key", DurableProviderKey.Create(request.ScopeId, workId.Value));
        ...
        command.Parameters.AddWithValue("jitter_seed", DurableProviderKey.CreateJitterSeed(request.ScopeId, workId.Value));
        command.Parameters.AddWithValue("initial_retry_delay", request.RetryPolicy.InitialRetryDelay);
        command.Parameters.AddWithValue("retry_after_cap", request.RetryPolicy.MaximumRetryDelay);
    ...
    {
        const string sql = """
            SELECT work_id, command_id, revision, accepted_at, request_sha256
            FROM appsurface_durable.work
            WHERE scope_id = @scope_id
        ...
        if (matches.Count != 1 || !matches[0].RequestFingerprint.AsSpan().SequenceEqual(fingerprint))
        {
            return DurableOperationResult<DurableWorkAcceptance>.Failure(new DurableProblem(
