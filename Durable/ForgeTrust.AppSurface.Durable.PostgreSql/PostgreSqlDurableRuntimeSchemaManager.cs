...
public sealed class PostgreSqlDurableRuntimeSchemaManager : IDurableRuntimeSchemaManager
{
    ...
    /// <summary>Minimum delay between failed migration-lock attempts.</summary>
    internal const int MigrationLockRetryMinimumMilliseconds = 75;

    /// <summary>Maximum delay between failed migration-lock attempts.</summary>
    internal const int MigrationLockRetryMaximumMilliseconds = 125;
    ...
    private readonly Func<int> _sampleMigrationLockRetryDelayMilliseconds;
    private readonly Func<TimeSpan, CancellationToken, Task> _delayMigrationLockRetry;
    ...
    /// <param name="sampleMigrationLockRetryDelayMilliseconds">Optional deterministic sampler for the inclusive 75–125 ms production jitter range.</param>
    /// <param name="delayMigrationLockRetry">Optional asynchronous delay seam for lock polling tests.</param>
    ...
    internal PostgreSqlDurableRuntimeSchemaManager(
        ...
        Func<int>? sampleMigrationLockRetryDelayMilliseconds = null,
        Func<TimeSpan, CancellationToken, Task>? delayMigrationLockRetry = null,
        ...
        Action<TimeSpan>? observeStatusConnectionAcquisition = null)
    {
        ...
        _sampleMigrationLockRetryDelayMilliseconds = sampleMigrationLockRetryDelayMilliseconds
            ?? (static () => Random.Shared.Next(MigrationLockRetryMinimumMilliseconds, MigrationLockRetryMaximumMilliseconds + 1));
        _delayMigrationLockRetry = delayMigrationLockRetry ?? Task.Delay;
    ...
    public string GenerateScript(int fromVersion = 0)
    {
        ...
        builder.Append("    lock_deadline timestamp with time zone := pg_catalog.clock_timestamp() + interval '")
            ...
            .AppendLine(" seconds';");
        builder.AppendLine("    v_retry_delay_ms integer;");
        ...
        builder.AppendLine("        v_retry_delay_ms := 75 + pg_catalog.floor(pg_catalog.random() * 51)::integer;");
        builder.AppendLine("        PERFORM pg_catalog.pg_sleep(LEAST(");
        builder.AppendLine("            v_retry_delay_ms / 1000.0,");
        builder.AppendLine("            GREATEST(EXTRACT(EPOCH FROM lock_deadline - pg_catalog.clock_timestamp()), 0)");
        builder.AppendLine("        ));");
    ...
    private async ValueTask AcquireMigrationLockAsync(NpgsqlConnection connection, CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (true)
            {
                ...
                var sampledMilliseconds = _sampleMigrationLockRetryDelayMilliseconds();
                if (sampledMilliseconds is < MigrationLockRetryMinimumMilliseconds or > MigrationLockRetryMaximumMilliseconds)
                {
                    throw new InvalidOperationException("The migration-lock jitter sampler returned a value outside the supported range.");
                }

                var sampledDelay = TimeSpan.FromMilliseconds(sampledMilliseconds);
                await _delayMigrationLockRetry(
                    remaining < sampledDelay ? remaining : sampledDelay,
    ...
    private TimeoutException CreateMigrationLockTimeoutException() => new(
        ...
        + MigrationAdvisoryLock.ToString(CultureInfo.InvariantCulture) + ". Retry after the active migration owner completes.");
