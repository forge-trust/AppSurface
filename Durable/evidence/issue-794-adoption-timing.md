...
`APPSURFACE_DURABLE_MEASUREMENT_TIMEOUT_SECONDS` to another whole-second bound from 1 through 86,400 when the
documented journey has a different target. A timed-out command and its process group are terminated and recorded as
`timeout`.
