...
Measured on 2026-08-06 from the `codex/durable-slice-7-adoption` working tree after a clean source build. The checked-in [`warm/run.json`](warm/run.json) records the immutable head and merge-base revisions alongside the working-tree source fingerprint used for the measurement.
...
| Source-bound warm PostgreSQL reference workload | 27 seconds | Passed 6 Work recovery scenarios plus forward migration rollback/retry; see [`warm/run.json`](warm/run.json). |
