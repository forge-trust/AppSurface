...
public sealed class ReleaseWorkflowPolicyTests
{
    ...
    public async Task ReleaseWorkflowsDeclareConcurrencyAndAvoidEval()
    {
        ...
        Assert.Contains("Allowing superseded release note stub", prep, StringComparison.Ordinal);
        Assert.Contains("must point at ./v${version}.md", prep, StringComparison.Ordinal);
        Assert.Contains("superseded release note stubs and sidecars only for redirect_aliases migration", prep, StringComparison.Ordinal);
