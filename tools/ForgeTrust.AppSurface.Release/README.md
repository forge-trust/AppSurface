...
./eng/release publish --version 0.1.0 --tag v0.1.0 --base-ref release/0.1.0 --dry-run --docs-catalog ./dist/docs/versions.json --docs-trusted-release-root ./dist/docs
```
...
`--fail-on-warnings` and `--allow-existing-targets` are intentionally check-only options. `--docs-catalog` and `--docs-trusted-release-root` are check and publish options only. `prepare` rejects all of these options so a maintainer does not think warning policy, target-collision policy, or docs archive selection changed while generating release artifacts.
...
`publish` is create-only. It validates that the supplied tag is annotated, resolves to a commit reachable from `origin/<base-ref>`, has a successful protected NuGet publish run for the tag's release classification, has no existing GitHub Release, and contains the versioned release note, sidecar, release manifest, and matching release evidence bundle at the tag commit before GitHub Release creation. Prerelease tags require `nuget-prerelease-publish.yml` proof. Stable tags require `nuget-stable-publish.yml` proof and verified docs archive evidence before GitHub Release creation, so `v0.1.0` cannot become a GitHub-only release or a stable release without catalog-pinned docs. The command writes `version`, `tag`, `tag_commit`, `note_path`, `notes_file`, `release_classification`, `evidence_path`, `evidence_subject_sha256`, `evidence_tag_commit`, `docs_release_manifest_sha256`, and `prerelease` outputs when `--github-output` is supplied. `--github-output` must be a file path, not a root directory; in GitHub Actions, pass the `GITHUB_OUTPUT` file supplied by the runner.

Stable publish requires explicit `--docs-catalog` input from the staged docs artifact. Unlike `check`, `publish` does not fall back to a local `dist/docs/versions.json` because the release workflow must verify the artifact it is about to publish, not whatever happens to be in the checkout.
...
./eng/release check --version 0.1.0 --allow-existing-targets --fail-on-warnings --docs-catalog ./dist/docs/versions.json --docs-trusted-release-root ./dist/docs
./eng/release publish --version 0.1.0 --tag v0.1.0 --dry-run --docs-catalog ./dist/docs/versions.json --docs-trusted-release-root ./dist/docs
```
...
- `release-docs-catalog-input-missing`: pass the staged docs `versions.json`; for publish, do not rely on local fallback.
