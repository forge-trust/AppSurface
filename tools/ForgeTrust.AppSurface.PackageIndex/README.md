...
[`packages/package-index.yml`](../../packages/package-index.yml). The value is a finite reader-facing policy choice:
...
The canonical bodies live in [`release-guidance.md`](./release-guidance.md). Each body expands the package chooser and
...
1. Edit the finite variant or a manifest field in [`packages/package-index.yml`](../../packages/package-index.yml).
...
4. Verify without writing:
   ...
   marker policy without writing files. [Package-gate CI](../../.github/workflows/package-gate.yml) runs both commands.
