...
`packages/package-index.yml`. The value is a finite reader-facing policy choice:
...
The canonical bodies live in `release-guidance.template`. Each body expands the package chooser and
...
express the reader-facing posture, add one exact template pair in `release-guidance.template`, extend
