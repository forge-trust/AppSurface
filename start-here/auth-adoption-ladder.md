...
| I need a real local OIDC provider proof without signing up for SaaS or teaching Keycloak administration. | `ForgeTrust.AppSurface.Auth.Aspire.Keycloak` | `dotnet package add ForgeTrust.AppSurface.Auth.Aspire.Keycloak` in an AppHost project | AppHost-only Keycloak resource setup, deterministic local realm/client/users, optional digest-pinned login-theme source validation/mounting, immutable-image-ready build-contract evidence, secret-safe OIDC projection, readiness probes, fixed-port diagnostics, disposable-data default, and ASKEYC diagnostics. | Runtime web auth registration, production Keycloak administration or realm mutation, image build/push, deployment, social IdPs, confidential-client secret lifecycle, provider SDK ownership, tenant mapping, user provisioning, app-user mapping, token storage, or web/runtime dependency on Keycloak packages. | Run the [Auth Aspire Keycloak AppHost proof](../examples/auth-aspire-keycloak-apphost/README.md) and the [theme quickstart](../Auth/ForgeTrust.AppSurface.Auth.Aspire.Keycloak/README.md#theme-quickstart). | Keep the package in AppHost/dev/test projects; pair runtime web apps with `ForgeTrust.AppSurface.Auth.AspNetCore.Oidc` directly. | Override ports, callback paths, seeded users, persistent data, and `LoginTheme` only for local proof needs; keep production image and realm operations in application CI/runbooks. |
