...
| I need fake local personas to manually prove AppSurface policy behavior. | `ForgeTrust.AppSurface.Auth.AspNetCore.DevAuth` | `dotnet package add ForgeTrust.AppSurface.Auth.AspNetCore.DevAuth` | Development-only fake persona scheme, local control page, selected-persona cookie, marker overlay, safe local status JSON, and ASDEV diagnostics. | Production authentication, OIDC, JWT validation, ASP.NET Identity, password handling, durable app-user mapping, user stores, tenant authority, audit sinks, deployment auth, automatic host HTML injection, RazorWire auth UI components, or Auth.Testing integration-test assertions. | Run the [DevAuth example](../examples/auth-aspnetcore-dev-auth/README.md). | Remove DevAuth package references, `AddAppSurfaceDevAuth(...)`, `MapAppSurfaceDevAuth()`, and DevAuth scheme references before production. | Override scheme name, path prefix, cookie name, marker CSS, and default-scheme behavior only for local proof hosts. |
...
| I need RazorWire UI to reflect auth state. | Existing `ForgeTrust.RazorWire` proof surfaces; adapter package deferred unless present in the package chooser. | No install command for a RazorWire auth adapter in this guide. | Current proof can show auth result state rendered in a browser-facing page. | Shipped RazorWire auth gate components, endpoint enforcement, policy ownership, or production auth behavior. | Run the [Auth Web/RazorWire proof](../examples/auth-web-razorwire-proof/README.md). | Treat RazorWire-facing auth projection as future or branch-specific unless a package appears in `packages/package-index.yml`. | Keep enforcement in ASP.NET Core policies and AppSurface endpoint filters. |
...
7. Add DevAuth only to Development hosts where a visible fake-persona proof helps manual testing.
8. Remove DevAuth before production and keep real challenge, forbid, redirect, and sign-in/out behavior host-owned.
...
- DevAuth marker rendering in Development-only proof pages where fake auth must stay visible.
...
| DevAuth startup failure | Development fake auth refuses to start. | DevAuth detected production environment, real scheme conflicts, missing personas, bad path prefix, or invalid persona ids. | Read the ASDEV code and keep DevAuth Development-only and visibly fake. | [DevAuth diagnostics](../Auth/ForgeTrust.AppSurface.Auth.AspNetCore.DevAuth/README.md#diagnostics) |
