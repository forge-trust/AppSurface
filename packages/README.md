...
- [v0.1.0-rc.2 release note](../releases/v0.1.0-rc.2.md) is the current package-facing story for this coordinated release.
...
- Add `ForgeTrust.AppSurface.Flow` when you want typed process graphs and local runner tests.
...
- Install `ForgeTrust.AppSurface.Cli` when you want `appsurface docs` workflows or a private `appsurface coverage gate` without running from source; verify tool installs with `appsurface --version` or `dotnet tool run appsurface --version` for local manifests.
