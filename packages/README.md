...
- Install `ForgeTrust.AppSurface.Cli` when you want the `appsurface docs` repository preview command without running from source; verify tool installs with `appsurface --version` or `dotnet tool run appsurface --version` for local manifests.
