...
What you get: Base ASP.NET Core hosting, middleware and endpoint composition, conventional browser error-page hooks, and the core Runnable web module contract.
...
| `ForgeTrust.Runnable.Web` | Install this first for a normal ASP.NET Core app with Runnable modules when you want the base web startup pipeline before layering on optional UI or docs packages. | `dotnet package add ForgeTrust.Runnable.Web` | Base ASP.NET Core hosting, middleware and endpoint composition, conventional browser error-page hooks, and the core Runnable web module contract. | OpenAPI generation, Scalar UI, RazorWire reactive UI, Tailwind build integration, or the RazorDocs host. | [Package README](../Web/ForgeTrust.Runnable.Web/README.md) |
...
- Run `dotnet run --project tools/ForgeTrust.Runnable.PackageIndex/ForgeTrust.Runnable.PackageIndex.csproj -- verify-packages --package-version 0.0.0-ci.local` before publishing changes that affect package metadata, project references, or Tailwind runtime payloads.
