...
- `appsurface coverage run` now prints 30-second heartbeats and warns after 10 minutes without observable operation progress by default. Use `--watchdog fail` for bounded process-tree cleanup and exit `124`, or `--heartbeat-interval 0 --watchdog off` to preserve the previous silent, non-classifying behavior. See the [coverage-run watchdog guide](./Cli/ForgeTrust.AppSurface.Cli/README.md#coverage-run-watchdog).
