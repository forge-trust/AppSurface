...
- AppSurface CI coverage now dogfoods the source `appsurface coverage run` command for the default full-solution coverage lane, preserving the existing Cobertura, JUnit, slow-test diagnostics, Codecov, and `coverage gate` evidence paths.
...
- `appsurface coverage run` now supports `--test-results junit` for AppSurface-managed top-level JUnit artifacts, and `--slow-test-diagnostics` implies managed JUnit results for diagnostics. Standalone coverage merge, grouped CLI execution, packaged local-tool CI proof, and TRX/TUnit parsing remain follow-up work.
- Coverage runs now write slow-test diagnostics in Markdown and JSON, including project timings, parsed JUnit test-case evidence, best-effort parser warnings, metadata completeness, and diagnostic aggregation overhead in seconds and as a percent of runner time.
