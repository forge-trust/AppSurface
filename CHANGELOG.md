...
- Added `ForgeTrust.AppSurface.Config.LocalSecrets`, a local-only secret posture package with structured result states, fail-closed provider resolution, `appsurface secrets` commands, migration docs, and an executable example for solo development before a remote vault exists.
