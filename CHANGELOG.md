...
- [`ForgeTrust.RazorWire`](./Web/ForgeTrust.RazorWire/README.md#choose-who-supplies-turbo) now emits its exact Turbo 8.0.12 UMD runtime from the package and same application origin by default. Apps can instead select a validated same-origin custom path or explicitly take full host-managed ownership of Turbo sourcing and load order.
