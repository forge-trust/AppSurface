...
- Group public JavaScript doclets into explicit AppSurface Docs API families with ordered source-path naming rules, namespace/module precedence, and path-aware fallback routes.
...
- RazorWire CLI process execution now has an explicit reliability contract for captured commands, launched target-app output, startup diagnostics, cancellation, and cleanup while preserving the existing export command surface.
