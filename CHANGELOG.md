...
- Release publishing now creates the durable public docs archive, `.sha256` digest, Pages catalog entry, recovery summary, and public verification gate before a draft GitHub Release can become public. Later main docs deploys hydrate existing release archives first, so published release docs stay available after ordinary docs updates.
