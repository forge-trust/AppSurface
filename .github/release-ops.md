...
## Tailwind artifact provenance (issue #798)

For a coordinated plan containing Tailwind, the [provenance reference](../docs/tailwind-artifact-provenance.md) defines the required producer-to-push proof. The protected workflows implement that contract; the [PackageIndex maintainer guide](../tools/ForgeTrust.AppSurface.PackageIndex/README.md#tailwind-artifact-provenance-798) documents the typed CLI.

The producer validates the tag checkout, package plan and source; performs its local consumer proof; packs once; and uploads one immutable bundle containing `package-artifact-manifest.json` v1 and `tailwind-proof-subject.json`. Its protected outputs are producer artifact ID and exact subject SHA-256. Native jobs download by that ID and use the action-returned download path; the same ID is passed into the verifier and written to each receipt. Do not select by latest artifact name, trust IDs/digests read from receipts, or repack after a successful producer upload.

The typed `verify-tailwind-consumer --mode release` proves actual restored archives and extracted payload against the producer bundle on exactly `linux-x64`, `linux-arm64`, `osx-x64`, `osx-arm64`, and `win-x64`. The producer's successful `net10.0` graph plus validated inventory freezes first-party membership (Core and Tailwind); assets metadata locates the selected packages, while direct archive SHA-512 and payload comparison prove bytes. Five successful v2 receipts (`appsurface-tailwind-native-host-proof-v2`) for one invocation are mandatory. Local mode and historical v1 receipts cannot satisfy this gate. If any host fails or only one matrix leg was rerun, rerun all five native jobs (or the full release workflow) so the aggregate contains one complete invocation; it reuses the frozen producer. Do not use failed-jobs-only reruns to mix attempts.

The main-only manual dispatch of [Package Artifacts](./workflows/package-artifacts.yml) also rehearses interrupted publication with the actual downloaded producer, five-host aggregate, uploaded start receipt, and prepared package bytes. It validates those authorities on both attempts, then passes the would-be NuGet commands to a recording runner with a synthetic key. The first Tailwind push is reported as interrupted and the second as a duplicate; neither attempt reads a release token or reaches NuGet. Ordinary local test runs skip this artifact-dependent case.

Before requesting or reading NuGet credentials, publishing downloads the exact producer and successful aggregate by immutable IDs, runs `publish-preflight`, prepares isolated push inputs, and uploads or recovers the authoritative publication-start receipt. That receipt becomes authority only after upload succeeds and returns its artifact ID. The publisher validates it and the same evidence again before the first push, then rehashes each package file immediately before pushing. If a start receipt exists, recover its exact original producer and aggregate IDs first; a newer aggregate cannot replace them. If a push may have succeeded before its ledger update, treat the outcome as unknown until observed. `duplicate-reported` is not remote byte-equality evidence.

Upload producer, host evidence, aggregate, start receipt and ledger using the validated configured `github.retention_days`; do not invent a shorter override or promise unsupported minimum days. Include the actual earliest expiry among required original artifacts and the remaining workflow rerun eligibility in diagnostics. GitHub permits reruns for 30 days and up to 50 reruns, so the supported same-run deadline is the earlier of required-artifact expiry and the 30-day run window; report remaining reruns too. Retaining artifacts longer does not extend the rerun window. A new producer requires new five-host proofs; an unchanged original producer may be reused for full native rerun, and a successful aggregate may be reused for publish-only retry. Once start receipt exists, its original IDs govern.

If the original producer, aggregate or start receipt is expired, missing, unreadable, duplicated, or its run history is unknown, stop same-run recovery. Never create a replacement bundle under the original receipt or infer that no push happened from missing history. Follow the existing fix-forward/abandonment policy; content defects after any coordinated package is accepted require a new version. Preserve all available failed-host diagnostics. See the reference's [failure and recovery table](../docs/tailwind-artifact-provenance.md#failure-and-recovery) before acting.
