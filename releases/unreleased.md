...
- AppSurface now has a protected, tag-only NuGet prerelease publish workflow that revalidates package artifact manifests, uses NuGet Trusted Publishing instead of a long-lived API key, requires a reviewed `nuget-prerelease` environment, writes a redacted publish ledger, and smoke-restores published packages from a clean NuGet configuration before a prerelease is considered ready.
