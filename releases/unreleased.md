...
- `ForgeTrust.AppSurface.Aspire.Testing` now closes Aspire 13.4.4's partial-provider cleanup gap for profile tests. The builder captures the pinned Aspire root provider immediately before host resolution, disposes it when host construction fails, preserves the original non-process-fatal build exception when cleanup also fails, and transfers ownership unchanged to the returned application after a successful build. A registration-shape guard fails closed when the pinned integration point changes.
