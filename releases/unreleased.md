...
- RazorWire stream subscriptions now deny by default through `RazorWireOptions.Streams.AuthorizationMode = DenyAll`, with an explicit `AllowAll` mode for public/demo streams and `IRazorWireChannelAuthorizer` preserved as the request-aware extension point for user, tenant, and workflow-specific channels.
