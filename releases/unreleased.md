...
- `ForgeTrust.AppSurface.Web.OpenApi` now uses `Microsoft.AspNetCore.OpenApi` 10.0.9 and directly requires `Microsoft.OpenApi` 2.7.5 or later, moving the default 2.x resolution above the range affected by [GHSA-v5pm-xwqc-g5wc](https://github.com/advisories/GHSA-v5pm-xwqc-g5wc) while preserving existing OpenAPI and Scalar APIs and endpoint behavior. Consumers overriding the coordinated graph to 3.x must use 3.5.4 or later and retest because 3.0.0 through 3.5.3 is also affected.
