...
- AppSurface now publishes the public `appsurface` .NET tool for repository-level workflows, starting with `appsurface docs` for RazorDocs preview and `appsurface docs export` for repo-owned RazorDocs static export. RazorWire-specific export workflows remain owned by the separate `razorwire` tool.
...
- RazorDocs now permanently redirects public Markdown source-shaped requests such as `/docs/packages/README.md` to the clean canonical route, so links copied from GitHub or editor paths recover instead of falling into the generic 404 page.
...
- RazorDocs code-fence language badges now render as package-owned chrome instead of inline copied text, keeping search indexing and clipboard behavior focused on the authored code body while preserving visible language labels.
