...
- `ForgeTrust.AppSurface.Auth.AspNetCore.DevAuth` adds a Development-only persona lab with a named fake auth scheme, protected persona cookie, loopback-only control page, status JSON, embeddable state marker overlay, and startup guards so package consumers can prove AppSurface auth flows without shipping fake identity.
...
- `ForgeTrust.AppSurface.Auth.AspNetCore.DevAuth` documents the five-minute local persona flow, persistent in-app marker overlay, marker skinning options, DevAuth versus real host auth/OIDC/test harness boundaries, stable `ASDEV###` diagnostics, and removal guidance before deployment.
