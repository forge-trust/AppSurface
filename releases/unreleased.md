...
- RazorDocs search now carries a `Search Mode` facet so contributor and benchmark material can stay out of default navigation and default search while remaining discoverable through explicit modes, while test-project docs and generated example-app API reference are excluded from the harvested docs surface.
