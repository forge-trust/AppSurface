...
- [`ForgeTrust.AppSurface.Durable`](../Durable/ForgeTrust.AppSurface.Durable/README.md) now separates passive adopter contracts from the public [`ForgeTrust.AppSurface.Durable.Provider`](../Durable/ForgeTrust.AppSurface.Durable.Provider/README.md) SPI, with versioned command fingerprints, validated opaque ids, immutable result boundaries, enforceable Worker execution-identity transitions, bounded Cron inputs, passive registration proof, and checked-in public-API baselines. Both source-preview packages remain machine-held from publication until the PostgreSQL provider milestone supplies conformance and restore evidence.
