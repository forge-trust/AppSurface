...
- Prerelease smoke install now waits on a dedicated GitHub environment before allocating a runner, then restores public library packages through one aggregate smoke project so NuGet validation and indexing delays do not multiply across the package surface.
...
- File-backed configuration audit sources now include optional line and UTF-8 byte-column coordinates when the file provider can prove the location matches the parsed and merged source record; ambiguous or unsupported paths keep the existing file/config-path provenance without a misleading coordinate.
...
- Config audits can now opt individual dictionary entries into scoped HMAC key correlation. Redacted labels such as `[redacted-key-1]` remain report-local, while `ConfigAuditElementIdentity.KeyCorrelationId` gives support engineers an opaque, key-scoped comparison id when deployment-local `ConfigAuditDictionaryKeyCorrelationOptions` are configured.
...
- AppSurface Docs can now require complete public JavaScript event doclets during release verification with `AppSurfaceDocs:Harvest:JavaScript:RequireCompleteEventDoclets` and `appsurface docs verify-health`, turning missing `@target`, `@firesWhen`, or either at least one valid `detail.*` property or `@detail none` into blocking diagnostics without changing default warning-only harvesting. `@example`, `@bubbles`, and `@cancelable` remain recommended authoring fields, but are intentionally excluded from the blocking strict-event contract.
...
- AppSurface Docs search-index refresh is now a POST-only operator action at `{DocsRootPath}/_search-index/refresh`, gated by anti-forgery validation and a host-owned authorization policy while legacy `?refresh=1` reader URLs remain read-only.
