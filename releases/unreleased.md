...
- AppSurface now has a repository-owned release cockpit at `./eng/release`, with `check`, `prepare`, and `publish` commands that generate tagged release notes, sidecar metadata, release manifests, package release-note path updates, changelog rollover, reset unreleased artifacts, readiness reports, and GitHub Actions outputs from the tag commit.
- Release publishing now fails closed for stable versions until a protected stable NuGet publish path exists, so `v0.1.0` cannot become a GitHub-only release while public packages remain unpublished.
...
- Release preparation pull requests can now be generated by a maintainer-dispatched workflow and rechecked by a read-only `pull_request` review job that recomputes release warnings without `pull_request_target` or repository secrets.
