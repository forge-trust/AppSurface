...
- `ForgeTrust.AppSurface.Auth.Testing` adds a public test-only integration harness with deterministic personas, WebApplicationFactory helpers, per-request persona selection, diagnostics, and AppSurface auth assertion helpers while keeping real ASP.NET Core policy evaluation in the path.
...
- `ForgeTrust.AppSurface.Auth.Testing` documents the under-five-minute WebApplicationFactory path, scheme modes, persona registry rules, subject mapping defaults and overrides, diagnostics, assertion helpers, and troubleshooting for integration-test consumers.
