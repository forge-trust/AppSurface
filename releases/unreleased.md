...
- RazorDocs search now renders starter query URLs and browse recovery links in the server shell, so no-JS readers, crawlers, static `search.html` exports, and failed `search-index.json` fetches still have real documentation paths to follow.
