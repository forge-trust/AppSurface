...
4. **Two-phase host scenario.** Build `DurableHostScenario` from explicit health, pump admission, request, and `TimeProvider` inputs. `AssessHealthAsync` returns a completed health observation. `RunDirectPumpOnceAsync` requires a completed assessment, captures its latest snapshot, invokes authoritative admission exactly once regardless of the snapshot's advisory `CanAttemptPump`, and returns the exact attempt. Use one deadline for each observation and one overall deadline; return completed negative outcomes immediately. Preserve refusal, unavailability, and incompatibility as provider attempt outcomes. No polling, retries, HTTP mapping, or independent readiness policy.
...
- Scenario tests prove all six assessed health states still call authoritative admission once, including NotStarted, Stale, Draining, Incompatible, and Unavailable; the provider's exact Completed, Refused, Unavailable, or Incompatible attempt is returned without synthesis. They distinguish a completed empty pass, pre/post-invocation cancellation, observation timeout, and overall timeout.
...
- Define “latest assessment” as the most recently completed successful assessment, published under synchronization. A pump call atomically captures that snapshot; one snapshot may be reused for multiple pump calls. Missing assessment throws an explicit `InvalidOperationException`; failed/canceled assessments never replace the last successful one. An in-budget, uncanceled pump invocation calls authoritative admission exactly once regardless of advisory `CanAttemptPump`; only the provider attempt determines admission.
...
- Specify a public `DurableScenarioPumpInvocation` handle with a per-scenario sequence, exact request and captured assessment, and a `Task<DurableRuntimePumpAttempt> Completion` that can be awaited repeatedly. Record the handle before calling admission, convert the provider `ValueTask` to that `Task` exactly once, and retain handles in an atomic `PumpInvocations` snapshot for the scenario instance's lifetime. A post-invocation `DurableScenarioTimeoutException` exposes the handle; a pre-invocation timeout has no handle. The scenario deadline ends only its wait and never cancels the provider token; caller cancellation remains independent. A synchronous admission exception faults `Completion` and propagates unchanged. Awaiting `Completion` later yields the exact attempt, canceled-task semantics with the caller token, or the original non-cancellation exception; a fault after a permit still needs provider recovery to establish effect status. The scenario has no disposal method that cancels or hides an in-flight call; reuse leaves prior handles reachable. Document test-instance retention and the caller's obligation to await timed-out calls. Test a late success, late exception, ignored cancellation, synchronous exception, reuse, and handle retrieval after timeout.
- Pass only the caller's `CancellationToken` to provider admission; the scenario's `TimeProvider` deadline controls waiting and is never linked into that token. At one synchronized terminal check, an already-completed provider task wins, then observed caller cancellation, then an expired overall budget, then an expired per-observation budget. Expose `DurableScenarioTimeoutException.Reason` as `Overall` or `Observation`, its `Phase` as health or pump, and its optional `Invocation`; never report a fabricated pump attempt. After timeout, `Invocation.Completion` is the source of truth. Test completion/cancellation/deadline ties with gated tasks and a fake clock, plus a provider that honors and one that ignores caller cancellation.
- Start the shared overall budget at the first `AssessHealthAsync` invocation; concurrent assessments and all later pump calls on the same scenario share it. It never resets on reuse: calls at or after expiry time out before provider invocation; use a new scenario for a new budget. Publish only a provider result accepted by the assessment method's terminal decision point; assign its completion sequence under a lock at publication and have each pump atomically capture that entry. A provider result arriving after an assessment timeout is observed internally but never published. Failed/canceled assessments leave the latest successful one intact. Test reversed health completion, concurrent pumps, and reuse after expiry.
- Keep scenario `PumpInvocations` and fake histories as separate shallow retention scopes. Add `ClearCompletedPumpInvocations()` that atomically removes only terminal scenario handles; in-flight handles remain reachable until terminal and may then be cleared explicitly. No automatic eviction or scenario disposal hides an in-flight task. Document that callers must await timed-out handles before releasing provider/test resources and clear completed handles when retaining a scenario. A fake checks cancellation before invoking a delegate, passes the caller token unchanged, and records the delegate's actual terminal result or exception; a token requested during an ignoring delegate does not overwrite its result. Validate the six-state builder matrix against production predicates and constructor boundaries. Test clear/reuse races, mutable payload references, honoring/ignoring delegates, synchronous throws, and the packed dependency graph.
- Measure all budgets with `TimeProvider.GetTimestamp()` and `GetElapsedTime()`, never `GetUtcNow()`; wall time is for display only. Require custom time providers to supply monotonic timestamps. At the health terminal decision point, a completed successful read publishes once under the scenario lock; a provider fault or caller cancellation propagates, and a read completing after the method has timed out is observed but does not publish. The same `DurableScenarioTimeoutException` represents health timeout with `Phase = Health`, the exact `Reason`, and `Invocation = null`. Test wall-clock adjustments, deadline equality, late health completion/fault, caller cancellation, and both timeout reasons.
- Create the invocation handle and its completion source before calling admission. Convert a returned `ValueTask` to a task exactly once; synchronous throws complete the source before the original exception is rethrown. `Completion.IsCanceled` represents a provider `OperationCanceledException` associated with the caller's canceled token, while `Completion.IsFaulted` retains the original non-cancellation exception (and an unrelated cancellation exception as a fault). Document these states and test synchronous/asynchronous failure, caller cancellation, and later awaiting the same handle more than once.
- Define an immutable history snapshot as a frozen order and frozen entry metadata, not a deep copy of the exact request or payload references. Document that callers must not mutate payloads while asserting captured references; freeze safe scalar request facts separately when needed. Test that metadata remains unchanged if an underlying payload object is mutated and that no payload value appears in formatted diagnostics.
<!-- /autoplan-accepted:eng -->
## Review record
...
- [ ] **E1 (P1, human: ~2h / agent: ~30min)** — Scenario — Specify post-invocation timeout evidence, a retained invocation handle, and late-task observation. Surfaced by the initial and post-approval engineering reviews. Verify with fake-clock ignored-cancellation, late-success/error, handle-retrieval, and PG post-permit tests.
...
- [ ] **E6 (P1, human: ~1h / agent: ~20min)** — Scenario — Use monotonic timestamps and publish health only at its accepted terminal decision. Surfaced by final engineering pass. Verify wall-clock jumps, late health completion, and simultaneous deadlines.
- [ ] **E7 (P1, human: ~1h / agent: ~20min)** — Scenario — Preserve provider completion, cancellation, and fault states in retained handles. Surfaced by final engineering pass. Verify synchronous and asynchronous errors and repeated awaits.
- [ ] **E8 (P2, human: ~30min / agent: ~10min)** — Fakes and docs — Distinguish frozen history metadata from mutable payload references. Surfaced by final engineering pass. Verify mutation and safe diagnostics.
...
- **Advisory health versus authoritative admission:** CEO and Eng independently found that the confirmed scenario gate conflicts with production guidance. DX found the same tension in the first-use example. The user approved direct authoritative admission on 2026-09-24; the assessment remains observable but no longer gates the call.
...
- **Public API breadth and adoption evidence:** CEO challenged package size, DX required a five-minute packed quick start, and Eng requires artifact-level graph proof. The user approved the full package scope on 2026-09-24; packed adopter evidence remains a release criterion.
...
### Approval resolution (2026-09-24)

The user replied “A - approved w/ the rec changes.” This approves the full #802 Testing package and the review's recommended correction to the scenario: a completed assessment is required for the two-phase test flow, but `CanAttemptPump` is advisory and never a check-then-act gate. When a pump operation is within its deadline and has not been canceled, the scenario calls `IDurableRuntimePumpAdmission.TryRunOnceAsync` exactly once and preserves its exact attempt. No health state is converted into a synthetic refusal, unavailability, or incompatibility result. The implementation and engineering test artifact above carry this decision. The earlier CEO, DX, and Eng sections remain historical review records; their “unresolved” language describes the pre-approval state.

### Engineering rerun after approval

The first rerun reviewed the amended implementation snapshot `53685da3…cf9a9c` and found one P1 gap: the plan identified an in-flight operation after timeout but did not define how a caller could retrieve its eventual outcome. The accepted remedy is a per-invocation handle with a repeat-awaitable completion task, retained in a scenario history and exposed by the post-invocation timeout. Timeout affects the scenario's wait, while caller cancellation and the provider's result remain authoritative. A late fault still requires real-provider recovery to establish the status of external effects. The [engineering test plan](./issue-802-durable-testing-test-plan.md) now includes late success, late error, synchronous error, ignored cancellation, handle retrieval after scenario reuse, and post-permit ambiguity. The outside process remains unavailable under Codex-host preflight; no cross-model consensus is claimed. A final native engineering pass is running against the complete revised implementation.

The second rerun reviewed snapshot `ca1056a5…8de50c5` and challenged token ownership, timeout/completion ties, retained payload references, shared-budget reuse, fake cancellation races, builder-default drift, and packed dependency verification. The accepted engineering block now specifies caller-token forwarding, a terminal-check order, per-scenario budget lifetime, completed-handle clearing, fake-delegate cancellation semantics, and corresponding tests. The full Testing package and direct authoritative admission remain the approved choices. A third native pass is checking this final revision.

The third native pass reviewed snapshot `afeae5a2…67c37f3c` and found five implementation-detail gaps: monotonic timestamp use, health publication after a timeout, handle fault/cancellation semantics, shallow-history wording, and health-phase error mapping. The accepted engineering block and test artifact now define each. This was the skill's third bounded rerun, so the final clarifications were not sent for a fourth independent pass. They remain explicit verification tasks for implementation. The outside Codex process was unavailable throughout; these findings are attributed to the native reviewer only.
...
| 14 | Approval | Use authoritative admission regardless of advisory health | User-approved recommendation | Correctness | Matches the production admission contract and avoids check-then-act behavior | retain the precheck gate |
| 15 | Approval | Keep the full public Testing package scope | User-approved recommendation | Completeness | Satisfies confirmed #802 scope, subject to packed consumer proof | stage a narrower package |
| 16 | Eng rerun | Expose retained invocation handle and late completion | Mechanical | Correctness | Timeout after admission begins cannot prove no work ran | orphan the provider task |
| 17 | Eng rerun | Separate caller token from scenario wait deadline | Mechanical | Explicit over clever | Preserves the provider cancellation contract and known race order | link the deadline into admission |
| 18 | Eng rerun | Publish health only at the accepted terminal decision | Mechanical | Correctness | A late health result must not silently replace the assessment after timeout | publish on raw provider completion |
| 19 | Eng rerun | Use monotonic timestamps and typed timeout reasons | Mechanical | Completeness | Wall-clock jumps and simultaneous expiries must be deterministic | compute budgets from UTC wall time |
| 20 | Eng rerun | Preserve canceled/faulted handle states and shallow-history limits | Mechanical | Correctness | Callers need the eventual result while payload references remain live | claim deep immutability or hide errors |

## GSTACK REVIEW REPORT

| Review | Trigger | Why | Runs | Status | Findings |
| --- | --- | --- | ---: | --- | --- |
| CEO Review | `/autoplan` | Scope and strategy | 1 | Approved | Full package retained; user approved authoritative admission |
...
| Eng Review | `/autoplan` | Architecture and tests | 4 native passes | Approved with verification tasks | Admission, deadline, history, and package boundaries reviewed; final clarifications await implementation tests |
| Design Review | `/autoplan` | UI and visual design | 0 | Skipped | No UI scope |
| DX Review | `/autoplan` | Developer experience | 1 | Approved with implementation tasks | 5/10 to 8/10 planned completeness; first-assertion time unmeasured, five-minute target |

- **OUTSIDE COVERAGE:** CEO, DX, and Eng outside Codex passes unavailable under this Codex host. CEO and DX had one completed native `combo/sub` review each; Eng had an initial pass and three bounded reruns. No cross-model consensus is claimed.
- **VERDICT:** The user approved the full #802 plan with direct authoritative admission on 2026-09-24. The final Eng pass's five clarifications are recorded in the implementation and test plan; the skill's three-rerun cap prevented another independent pass. The plan is approved for implementation, whose tests and packaging gates remain to be run. No #802 code, package, or release has been produced.
