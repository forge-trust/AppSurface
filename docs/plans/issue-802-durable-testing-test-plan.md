...
| Scenario handle retention | ClearCompletedPumpInvocations removes terminal handles atomically but preserves in-flight handles | Clear/complete race, exact pump-request reference lifetime, provider resources stay available until handles finish |
| Histories and native envelope | Atomic immutable scenario history ordered by invocation start; native envelope retains its typed payload reference | Snapshot/clear race, in-flight call from prior generation stays absent, envelope payload mutation remains visible through the retained reference, payload values never formatted in scenario history |
