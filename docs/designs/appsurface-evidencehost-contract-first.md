<!-- /autoplan restore point: /Users/andrew/.gstack/projects/forge-trust-Runnable/main-autoplan-restore-20260819-033309.md -->
...
`doctor` has three human-facing outcomes: `ready`, `ready_with_external_prerequisites`, and `blocked`. `ready_with_external_prerequisites` is not a hidden pass: it names the optional-but-not-currently-runnable producers and the command that would become blocked. `blocked` reports the first decisive problem and a concrete fix, for example “PostgreSQL evidence is required by profile `pr-persistence`; start the declared disposable dependency or select the documented CI environment.” An invalid policy, ambiguous diff, or invalid envelope is a stable diagnostic failure, not a prerequisite hint.
