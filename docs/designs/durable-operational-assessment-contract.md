<!-- /autoplan restore point: /Users/andrew/.gstack/projects/forge-trust-AppSurface/main-autoplan-restore-20260910-055206.md -->
...
11. Keep implementation provider-first, but do not freeze `IDurableRuntimePumpAdmission` until an executable packed consumer directly exercises every attempt kind and verifies the no-execution distinction.
...
Approved approach: retain one #794 case but implement it in reviewable stages, with a green legacy matrix after every stage. Keep provider-first sequencing, but require an executable packed-consumer exercise before the public admission API is frozen.
...
3. Keep provider-first sequencing and require an executable packed-consumer exercise before freezing `IDurableRuntimePumpAdmission`; the host service remains owned by #804.
...
| Scope calibration correct? | Prefer split slices or host-first proof | Scope bundles independent value | Confirmed concern; resolved by staged #794 delivery plus executable-consumer proof |
...
Product type is a public-preview .NET SDK/provider with CLI-managed PostgreSQL schema and an executable local example. Review mode is **DX POLISH**: keep the approved upstream boundary and make its consumption obvious, diagnosable, and safe.
...
The open repository, package READMEs, executable example, changelog, issue tracker, and contribution conventions already provide the ecosystem surface needed for this preview. The task guide links to the canonical issue/discussion route and identifies which concerns belong to #801, #802, #804, and #805. A plugin marketplace, new support channel, pricing/free-tier work, or another provider is not justified by this case.
...
- The executable PostgreSQL example, package/repository documentation, troubleshooting, and release notes.
...
The QA artifact is `/Users/andrew/.gstack/projects/forge-trust-AppSurface/andrew-main-eng-review-test-plan-20260910-073337.md`.
...
+ command seam  + hosted budget   + atomic health/security
    ...
    └───────────────┴─────────────────┘
                    │
Stage 4: mixed-version + scale + restricted-role + executable-consumer proof
                    │
...
| New structural decisions | 3 final-gate choices resolved: one staged case, computed provenance, provider-first with executable-consumer API-freeze gate |
...
3. **Keep provider-first sequencing, with a mandatory executable consumer exercise before API freeze.** This proves the admission switch and execution-certainty language in a real composition without pulling #804’s host service into #794.
